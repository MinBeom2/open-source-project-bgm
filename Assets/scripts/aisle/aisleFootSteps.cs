...
public class aisleFootSteps : MonoBehaviour
{
    ...
    [SerializeField] private aisleMovement movement; // Movement 스크립트 참조
    ...
    private void Update()
    {
        ...
        if (movement.CharacterController.isGrounded && movement.CharacterController.velocity.magnitude > 0)
        {
            ...
            if (movement.moveMode == aisleMovement.MoveMode.Run && actualInterval >= runStepInterval)
            {
                Debug.Log("뛰는 중");
                CheckGround();
            }
            else if (movement.moveMode == aisleMovement.MoveMode.Walk && actualInterval >= walkStepInterval)
            {
                Debug.Log("걷는 중");
                CheckGround();
            }
